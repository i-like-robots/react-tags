...
function filterSuggestions (query, suggestions, length, suggestionsFilter, noSuggestionsMessage) {
  ...
  let filtered = suggestions.filter((item) => suggestionsFilter(item, query)).slice(0, length)

  return (filtered.length > 0) ? filtered : [{ id: 0, name: noSuggestionsMessage, disabled: true }]
}
